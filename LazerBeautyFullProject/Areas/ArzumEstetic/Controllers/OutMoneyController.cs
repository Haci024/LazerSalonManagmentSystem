...
namespace LazerBeautyFullProject.Areas.ArzumEstetic.Controllers
{
    ...
    public class OutMoneyController : Controller
    {
        ...
        {
            List<OutMoney> outMoneys =_db.OutMoney.Include(x=>x.AppUser).Include(x=>x.SpendCategory).Where(x=>x.SpendCategory.FilialId==3).ToList();
        ...
        public async Task<IActionResult> UpdateOutMoney(int Id)
        {
           ...
           updateOutMoneyDTO.SpendCategories=_db.SpendCategories.Include(x=>x.Filial).Where(x=>x.FilialId==3 && x.Status==false).ToList();
        ...
        {
            List<SpendCategory> spendCategories = _db.SpendCategories.Where(x=>x.FilialId==3).ToList(); 
        ...
        public IActionResult AddSpendCategory(AddSpendCategoryDTO spendCategoryDTO)
        {
          SpendCategory spendCategory = new SpendCategory();
            bool SpendCategoryExists = _db.SpendCategories.Any(x => x.Category == spendCategoryDTO.Category && x.FilialId == 3);
