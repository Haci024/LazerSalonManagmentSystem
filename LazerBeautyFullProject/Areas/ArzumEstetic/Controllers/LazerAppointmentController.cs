...
namespace LazerBeautyFullProject.Areas.ArzumEstetic.Controllers
{
    ...
    public class LazerAppointmentController : Controller
    {
        ...
        {
            AppUser appUser = await _userManager.FindByNameAsync(User.Identity.Name);
            List<LazerAppointment> lazerAppointments =_appointmentService.NextSessionList(appUser);
        ...
        {
            AppUser appUser = await _userManager.FindByNameAsync(User.Identity.Name);
            List<LazerAppointment> lazerAppointments = _appointmentService.InComepletedList(appUser);
        ...
        {
            AppUser appUser = await _userManager.FindByNameAsync(User.Identity.Name);
            List<LazerAppointment> lazerAppointments=_db.LazerAppointments.Include(x=>x.Customers).Include(x=>x.LazerMaster).Include(x=>x.AppUser).Include(x=>x.LazerAppointmentReports).ThenInclude(x=>x.LazerCategory).Where(x=>x.IsCompleted==false && x.FilialId==appUser.FilialId ).ToList();
