...
namespace LazerBeautyFullProject.Areas.ArzumEstetic.Controllers
{
    ...
    public class IncomeController : Controller
    {
        ...
        public IActionResult IncomeMoneyList()
        {
            ...
            List<Income> incomeList = _db.Incomes.Include(x => x.AppUser).Include(x=>x.Stock).Where(x=>x.FilialId==3).ToList();
    ...
    InComeAddDTO inComeAddDTO = new InComeAddDTO();
            inComeAddDTO.Stocks = _db.Stock.Where(x=>x.RemainCount>0).Where(x => x.FilialId == 3).ToList();
        ...
        {
            inComeAddDTO.Stocks = _db.Stock.Where(x => x.RemainCount > 0 && x.FilialId == 3).ToList();
            ...
            income.Price =Convert.ToDecimal(inComeAddDTO.Price);
            income.BuyingPrice = Convert.ToDecimal(stock.BuyingPrice) * inComeAddDTO.Count;
        ...
        public IActionResult ArchiveProducts()
        {
            ...
            List<Stock> stock = _db.Stock.Include(x=>x.AppUser).Include(x=>x.Incomes).Where(x=>x.FilialId==3 && x.RemainCount==0).ToList();
