...
namespace LazerBeautyFullProject.Areas.ArzumBeauty.Controllers
{
    ...
    public class OutMoneyController : Controller
    {
        ...
        {
            List<OutMoney> outMoneys = _db.OutMoney.Include(x => x.AppUser).Include(x => x.SpendCategory).Where(x => x.SpendCategory.FilialId == 2).ToList();
        ...
        {
            List<SpendCategory> spendCategories = _db.SpendCategories.Where(x => x.FilialId == 2).ToList();
        ...
        public IActionResult AddSpendCategory(AddSpendCategoryDTO spendCategoryDTO)
        {
         ...
         bool SpendCategoryExists = _db.SpendCategories.Any(x => x.Category==spendCategoryDTO.Category && x.FilialId==2);
