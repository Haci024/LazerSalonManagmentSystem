...
namespace LazerBeautyFullProject.Areas.ArzumBeauty.Controllers
{
    ...
    public class IncomeController : Controller
    {
        ...
        public IActionResult IncomeMoneyList()
        {
            ...
            List<Income> incomeList = _db.Incomes.Include(x => x.AppUser).Include(x=>x.Stock).Where(x=>x.FilialId==2).ToList();
    ...
    InComeAddDTO inComeAddDTO = new InComeAddDTO();
            inComeAddDTO.Stocks = _db.Stock.Where(x=>x.RemainCount>0 && x.FilialId == 2).ToList();
        ...
        public IActionResult ArchiveProducts()
        {
            ...
            List<Stock> stock = _db.Stock.Include(x=>x.AppUser).Include(x=>x.Incomes).Where(x=>x.FilialId==2 && x.RemainCount==0).ToList();
