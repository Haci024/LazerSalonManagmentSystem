...
namespace LazerBeautyFullProject.Areas.ArzumMini.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public IActionResult SelectKassaActionListAddMoney(DateTime startDate, DateTime endDate)
        {
            ...
            return PartialView("~/Areas/ArzumMini/Views/Shared/_KassaActionList.cshtml", kassaActionLists);
        }
        public List<KassaActionList> GetKassaActionListAddMoney(DateTime startDate, DateTime endDate)
        {
            List<KassaActionList> kassaActionLists = _appDbContext.KassaActionLists.Include(x => x.Filial).Include(x => x.AppUser).Where(x => x.LastOutMoneyDate >= startDate && x.LastOutMoneyDate <= endDate && x.FilialId == 1 && x.Status == true).ToList();
        ...
        {
            List<KassaActionList> kassaActionLists = _appDbContext.KassaActionLists.Include(x => x.Filial).Include(x => x.AppUser).Where(x => x.LastOutMoneyDate >= startDate && x.LastOutMoneyDate <= endDate && x.FilialId == 1 && x.Status == false).ToList();
        ...
        public IActionResult SelectKassaActionListAllReports(DateTime startDate, DateTime endDate)
        {
            ...
            return PartialView("~/Areas/ArzumMini/Views/Shared/_KassaActionList.cshtml", kassaActions);
        }
        public List<KassaActionList> GetKassaActionAllReports(DateTime startDate, DateTime endDate)
        {
            List<KassaActionList> kassaActionLists = _appDbContext.KassaActionLists.Include(x => x.Filial).Include(x => x.AppUser).Where(x => x.LastOutMoneyDate >= startDate && x.LastOutMoneyDate <= endDate && x.FilialId == 1).ToList();
