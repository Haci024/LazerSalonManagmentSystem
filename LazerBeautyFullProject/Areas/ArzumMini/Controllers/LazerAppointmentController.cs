...
namespace LazerBeautyFullProject.Areas.ArzumMini.Controllers
{
    ...
    public class LazerAppointmentController : Controller
    {
        ...
        {
            var AppUser = await _userManager.FindByNameAsync(User.Identity.Name);
            List<LazerAppointment> lazerAppointments = await _appointmentService.NextSessionList(1);
        ...
        {
            var AppUser = await _userManager.FindByNameAsync(User.Identity.Name);
            List<LazerAppointment> lazerAppointments = await _appointmentService.InComepletedList(1);
        ...
        {
            var appUser = await _userManager.FindByNameAsync(User.Identity.Name);
            List<LazerAppointment> lazerAppointments = await _appointmentService.AllReservations(1);
        ...
        [HttpGet]
        public async Task<IActionResult> SessionTime(int AppointmentId)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> SessionTime(int AppointmentId, UpdateReservationTimeDTO updateReservationTimeDTO)
        {
            var validator = new UpdateReservationTimeValidator();
            var validationResult = validator.Validate(updateReservationTimeDTO);
            if (!validationResult.IsValid)
