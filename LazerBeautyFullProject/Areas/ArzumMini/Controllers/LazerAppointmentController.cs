...
namespace LazerBeautyFullProject.Areas.ArzumMini.Controllers
{
    ...
    public class LazerAppointmentController : Controller
    {
        ...
        {
            List<LazerAppointment> lazerAppointments=_db.LazerAppointments.Include(x=>x.Customers).Include(x=>x.LazerMaster).Include(x=>x.AppUser).Include(x=>x.LazerAppointmentReports).ThenInclude(x=>x.LazerCategory).Where(x=>x.IsCompleted==false).ToList();
